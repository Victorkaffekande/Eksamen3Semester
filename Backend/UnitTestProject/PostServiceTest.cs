...
public class PostServiceTest
{
    ...
    public PostServiceTest()
    {
        ...
        _mockRepo.Setup(x => x.GetPostGetById(It.IsAny<int>())).Returns<int>(id => _fakeRepo.FirstOrDefault(p => p.Id == id));
        _mockRepo.Setup(x => x.GetAllPostFromProject(It.IsAny<int>())).Returns<int>(id => _fakeRepo.FindAll(p => p.ProjectId == id));
        ...
        _mockRepo.Setup(x => x.DeletePost(It.IsAny<int>())).Callback<int>(s => _fakeRepo.Remove(_fakeRepo.Find(p => p.Id == s )));
        //Validator setup
...
// Valid create test
    ...
    public void CreatePostService_NullRepo()
    {
        ...
        //act + assert
        var ex = Assert.Throws<ArgumentException>(() => service = new PostService(null, _mapper, _postCreateDtoValidator,_postUpdateValidator));
    ...
    public void CreatePostService_NullMapper()
    {
        ...
        //act + assert
        var ex = Assert.Throws<ArgumentException>(() => service = new PostService(repo, null, _postCreateDtoValidator,_postUpdateValidator));
    ...
    public void CreatePostService_NullCreateValidator()
    {
        ...
        //act + assert
        var ex = Assert.Throws<ArgumentException>(() => service = new PostService(repo, _mapper, null,_postUpdateValidator));
    ...
    public void CreatePostService_NullPostValidator()
    {
        ...
        //act + assert
        var ex = Assert.Throws<ArgumentException>(() => service = new PostService(repo, _mapper, _postCreateDtoValidator,null));
        Assert.Equal("Missing Validator", ex.Message);
    }
    ...
    [InlineData(int.MinValue,"2000-11-11", "data:image/png;base64,filler", "filler", "Post must have a pattern Id")] // id is int min
...
/*
    ...
    */
    ...
    public void GetAllPostByProject_Test()
    {
        ...
        Assert.Contains(post3, result);
        Assert.DoesNotContain(post2,result);
